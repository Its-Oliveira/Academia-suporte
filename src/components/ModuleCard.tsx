...
export function ModuleCard({
  ...
  return <Card className={isLocked ? 'opacity-60' : ''}>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {!isLocked && progress > 0 && <ProgressBar value={progress} label="Progresso" />}

        {bestScore !== undefined && <div className="text-sm">
            <span className="text-muted-foreground">Melhor nota: </span>
            <span className={`font-semibold ${bestScore >= 70 ? 'text-success' : 'text-warning'}`}>
              {bestScore}%
            </span>
            {attempts > 0 && <span className="text-muted-foreground ml-2">
      ...
      <CardFooter>
        <Button onClick={handleStart} disabled={isLocked} className="w-full" variant={isCompleted ? "outline" : "default"}>
